...
class DefaultController extends Controller
{
    ...
    */
    ...
    {   
    	...
    	// total articles
        $query = $em->createQuery('SELECT COUNT(a.id) FROM SpljBundle:Article a WHERE a.status = 1');
        $articleCount = $query->getSingleResult();
        $stats->setTotalArticle($articleCount[1]);

		// total qcm
        $query = $em->createQuery('SELECT COUNT(m.id) FROM SpljBundle:Mcq m WHERE m.status = 1');
        $mcqCount = $query->getSingleResult();
        $stats->setTotalMcq($mcqCount[1]);

        // total questions
        $query = $em->createQuery('SELECT COUNT(q.id) FROM SpljBundle:Question q, SpljBundle:Mcq m WHERE m.id = q.idQcm AND m.status = 1');
        ...
        // chart articles
        $query = $em->createQuery(
            'SELECT COUNT(a) FROM SpljBundle:Article a WHERE a.status = 1 GROUP BY a.userId'
            );
        ...
        // chart mcq
        $query = $em->createQuery(
            'SELECT COUNT(m) FROM SpljBundle:Mcq m WHERE m.status = 1 GROUP BY m.userId'
            );
        ...
        // chart questions
        $query = $em->createQuery(
            'SELECT SUM(m.nbQuestions) FROM SpljBundle:Mcq m WHERE m.status = 1 GROUP BY m.userId'
            );
